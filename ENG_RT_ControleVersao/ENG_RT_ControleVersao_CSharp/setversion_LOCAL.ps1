...
#&  'C:\Program Files\MSBuild\14.0\bin\MSBuild.exe' C:\FonteDesenvolvimento\ENG_RT_ControleVersao\ENG_RT_ControleVersao_CSharp\ENG_RT_ControleVersao_WFA.csproj 
& "C:\Program Files (x86)\MSBuild\14.0\bin\msbuild.exe"  ((Get-Item -Path ".\.." -Verbose).FullName + "\ENG_RT_ControleVersao.sln")

& "git" add -A
& "git" commit -m ("incrementando versao LOCAL clsControleVersaoC#" + $versao) --quiet
